...
更新：2026-09-12。面向后续 Codex / AI 开发协作。开发设计、任务状态和验收标准见 `doc.md`，不要在这里维护第二份任务清单。
...
2026-09-13 用户授权将本轮小修、答辩材料和文档提交到既有 GitHub 私有仓库，要求保护隐私。测试报告已移除 Windows 个人目录名；教学参考文件只保留本地并加入忽略规则。提交继续使用 GitHub noreply 邮箱，真实凭据、数据库内容、录音、模型和 work 临时文件不入库；答辩截图来自既有虚构演示账号和合成语音验证。

2026-09-13：用户要求约 5 分钟、深色背景/大字/蓝色点缀的发布会式答辩展示，允许 HTML 形式。已制作 docs/defense 下 8 页离线 HTML、同版 PDF、逐页讲稿及使用说明，截图内嵌；代码构建文件在忽略目录 work/build_defense.mjs，预览验证在 work/defense-preview。未生成 PPTX，未改业务代码，未调用收费模型。材料区分历史 168 项全量和本轮小修 36 项针对性测试，说明合成语音来源，未声称自动长文本汇总或日志自主排障已实现。封面未代填个人身份，后续可按用户信息补充。操作服务仍需实时核实状态，演示文件本身可离线播放。
...
用户本次明确以答辩展示和少量 bug 修复为准，不为体验、生产要求或边缘情况扩大改动，不反复调用收费模型。question.md 保留其他 AI 原评估，顶部和末尾追加最新复核；不要按历史“待确认”事项重新追问或重测。会议 1063/任务 781 保留历史结果，不自动重分析；新提示词只影响之后的处理。
